...
class TestTransactionService(TestCase):
    def setUp(self):
        ...
        self.account = Account.create(
            address="0x123", name="Test Account", threshold=2, description="Test description"
        )
    ...
    def test_execute_transaction(self):
        ...
        # Set transaction status to approved (normally would happen in approve_transaction based on threshold)
    ...
    def test_transaction_list(self):
        ...
        all_transactions = TransactionService.transaction_list(
            account=self.account
        )
        ...
        assert all(tx.status == TransactionStatus.INITIATED for tx in initiated_transactions)
        ...
        assert all(tx.tx_type == TransactionType.TOKEN_SEND for tx in token_send_transactions)
