...
from spherre.app.models.transaction import Transaction, TransactionStatus, TransactionType
...
class TransactionService:
    ...
    def approve_transaction(
        ...
        if not transaction:
            raise ValueError(f"Transaction with ID {transaction_id} not found for this account")
        ...
        if transaction.status != TransactionStatus.INITIATED:
            raise ValueError(f"Transaction is in {transaction.status} state and cannot be approved")
        ...
        if transaction.proposer_id == member.id:
            raise ValueError("Transaction proposer cannot approve their own transaction")
    ...
    def execute_transaction(
        ...
        if transaction.status != TransactionStatus.APPROVED:
            raise ValueError(f"Transaction is in {transaction.status} state and cannot be executed")
        ...
        if approval_count < account.threshold:
            raise ValueError(
                f"Not enough approvals to execute. Got {approval_count}, need {account.threshold}"
            )
    ...
    def reject_transaction(
        ...
        if transaction.status not in [TransactionStatus.INITIATED, TransactionStatus.APPROVED]:
            raise ValueError(f"Transaction is in {transaction.status} state and cannot be rejected")
        ...
        # Example: if more than half of members reject, transaction is considered rejected
