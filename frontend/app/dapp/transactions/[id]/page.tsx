...
export default function TransactionDetailsPage({ params }: PageProps) {
  ...
  const { data: blockchainTransaction } = useGetTransaction(
    contractAddress,
    resolvedParams?.id ? BigInt(resolvedParams.id) : BigInt(0),
  )
  ...
  const currentApprovals =
    blockchainTransaction?.approved?.length ||
    transaction.approvals.filter((a) => a.status === 'Confirmed').length
  const requiredApprovals = Number(
    thresholdData?.[0] || transaction.threshold.required,
  )
  const canExecute =
    currentApprovals >= requiredApprovals && transaction.status === 'Pending'

  const approveTransaction = async () => {
    try {
      console.log('🔄 Approving transaction...')
      const result = await approveTransactionResult.writeAsync({
        tx_id: resolvedParams.id,
      })
  ...
  const rejectTransaction = async () => {
    try {
      ...
      const result = await rejectTransactionResult.writeAsync({
        tx_id: resolvedParams.id,
      })
  ...
  const executeTransaction = async () => {
    try {
      ...
      const result = await executeTransactionResult.writeAsync({
        tx_id: resolvedParams.id,
      })
      ...
      } else if (errorMessage.includes('insufficient approvals')) {
  ...
  return (
    <div className="min-h-screen bg-theme p-4 lg:p-6 transition-colors duration-300">
      ...
      {toastMessage && (
        <div
          className={`fixed top-6 right-6 z-50 px-6 py-4 rounded-xl shadow-2xl text-white font-medium transition-all duration-300 transform ${
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xl font-semibold text-theme-secondary">
                    {requiredApprovals}
                  </span>
              ...
              {!accountAddress ? (
                <div className="bg-theme-bg-secondary border border-theme-border rounded-xl p-6 text-center">
                  ...
                  <p className="text-theme-secondary mb-6 max-w-sm mx-auto">
                    Connect your wallet to manage this transaction and
                ...
                <div className="bg-theme-bg-secondary border border-theme-border rounded-xl p-6 text-center">
                  ...
                  <p className="text-theme-secondary max-w-sm mx-auto">
                    Only account members can manage transactions. Please contact
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    {transaction.status === 'Pending' && !canExecute && (
                      <div className="sm:col-span-2 lg:col-span-3 bg-theme-bg-secondary border border-theme-border text-theme-secondary px-6 py-4 rounded-xl font-medium flex items-center justify-center gap-3">
                        ...
                        <span>
                          Waiting for {requiredApprovals - currentApprovals}{' '}
                          more approval
