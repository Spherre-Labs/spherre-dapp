...
export default function MemberDetailsModal({
    ...
    return [
      {
        label: 'Proposed',
        color: '#FF8A25',
        value: fullDetails.proposed_count?.toString?.() ?? '0',
      },
      {
        label: 'Approved',
        color: '#FF7BE9',
        value: fullDetails.approved_count?.toString?.() ?? '0',
      },
      {
        label: 'Rejected',
        color: '#D44B4B',
        value: fullDetails.rejected_count?.toString?.() ?? '0',
      },
      {
        label: 'Executed',
        color: '#19B360',
        value: fullDetails.executed_count?.toString?.() ?? '0',
      },
    ]
  }, [fullDetails])
  ...
  const isLoading =
    addLoading || removeLoading || editLoading || perms.isLoading

  // History stats (currently unused in UI)
  useMemo(() => {
    if (!memberAddress) return
    const normalized = memberAddress.toLowerCase()
    const addCount = (addTxList || []).filter(
      (tx) =>
        (tx as { member?: string }).member?.toLowerCase?.() === normalized,
    ).length
    const removeCount = (removeTxList || []).filter(
      (tx) =>
        (tx as { member_address?: string }).member_address?.toLowerCase?.() ===
        normalized,
    ).length
    const editCount = (editTxList || []).filter(
      (tx) =>
        (tx as { member?: string }).member?.toLowerCase?.() === normalized,
    ).length
    return {
      total: addCount + removeCount + editCount,
      add: addCount,
      remove: removeCount,
      edit: editCount,
    }
        ...
        <div className="flex justify-between gap-4 mb-4">
          <div className="w-full">
            ...
            <h2
              ...
              className="text-xl text-center sm:text-[30px] font-bold"
            >
              Member Details
            </h2>
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-between mb-2 sm:mb-4 w-full">
          <div className="flex flex-row md:flex-col items-center col-span-1 gap-2">
            <div className="bg-theme-bg-tertiary border-[3px] md:border-[6px] border-[#6F2FCE] rounded-full overflow-hidden">
              <Image
                src={avatarSrc}
                alt={`User's image`}
                className="w-[40px] md:w-[130px] h-[40px] md:h-[130px] object-cover"
                width={130}
                height={130}
              />
            </div>
            <div className="flex flex-col">
              <span className="break-all text-base md:text-xl text-theme">
                {member.name}
              </span>
              <span className="text-theme-secondary text-sm md:text-base">
                {member.fullAddress.slice(0, 10)}...
                {member.fullAddress.slice(-4)}
              </span>
            </div>
          </div>
          <div className="grid grid-cols-2 mt-3 md:mt-0 gap-1 md:gap-2">
            {memberStats.map((stat) => (
              <div
                key={stat.label}
                className="bg-theme-bg-tertiary w-full md:w-[167px] h-[100px] md:h-[126px] rounded-[12px] px-0.5 py-0.5"
              >
                <div className="bg-theme-bg-secondary h-[80%] rounded-[10px] w-full">
                  <div className="rounded-[10px] p-2 md:px-4 md:py-[10px]">
                    <div className="flex items-center gap-2">
                      <div
                        className={`pl-[3px] pr-[2.5px] pb-[1px] rounded-[6px]`}
                    ...
                    <div className="font-bold mt-4 text-theme">
                      {stat.value}
                    </div>
                  </div>
                </div>
                <div className="text-sm px-2 text-theme-secondary">--</div>
              </div>
            ))}
          </div>
        </div>
