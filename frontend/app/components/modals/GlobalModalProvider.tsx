...
type ModalState =
  ...
  | { type: 'processing'; props: Omit<ProcessingModalProps, 'isOpen' | 'onClose'> }
  | { type: null };
...
interface GlobalModalContextType {
  ...
  showProcessing: (props?: Omit<ProcessingModalProps, 'isOpen' | 'onClose'>) => void;
  hideModal: () => void;
}
...
export const GlobalModalProvider = ({ children }: { children: ReactNode }) => {
  ...
  const showSuccess = (props: Omit<SuccessModalProps, 'isOpen' | 'onClose'>) => setModal({ type: 'success', props });
  const showError = (props: Omit<ErrorModalProps, 'isOpen' | 'onClose'>) => setModal({ type: 'error', props });
  const showProcessing = (props: Omit<ProcessingModalProps, 'isOpen' | 'onClose'> = {}) => setModal({ type: 'processing', props });

  return (
    <GlobalModalContext.Provider value={{ showSuccess, showError, showProcessing, hideModal }}>
      {children}
