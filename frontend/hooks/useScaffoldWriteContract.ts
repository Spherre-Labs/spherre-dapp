...
import type { ContractConfig, ContractWriteResult, ContractFunctionArgs } from "@/lib/contracts/types"
...
export function useScaffoldWriteContract({
  ...
  const { contract } = useContract({
    address: contractConfig.address.startsWith("0x") ? contractConfig.address as `0x${string}` : (`0x${contractConfig.address}` as `0x${string}`),
  ...
  const writeAsync = useCallback(
    async (args: ContractFunctionArgs = {}) => {
      ...
      try {
        // Convert args object to array format
        const calldata = Object.values(args)
        ...
        if (error instanceof Error) {
          ...
          } else if (msg.includes("contract not found") || msg.includes("class_hash_not_found")) {
    ...
    [contract, contractConfig.address, functionName, sendAsync, onSuccess, onError],
  )
