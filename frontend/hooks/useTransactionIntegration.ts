...
export function useTransactionIntegration(
  ...
  const processedTransactions = useMemo(() => {
    ...
    baseTransactions.forEach((baseTransaction: SpherreTransaction) => {
      ...
      try {
        ...
        if (transactionData) {
          try {
            const unifiedTransaction = transformTransaction(baseTransaction, transactionData)
          ...
          console.warn(`No transaction data found for transaction ${baseTransaction.id} of type ${baseTransaction.tx_type}`)
        }
  ...
  const enhancedError = useMemo(() => {
    ...
    return new Error('Unable to fetch transactions. Please check network or contract state.')
  }, [
    error,
    errorBase,
    errorToken,
...
export function useTransactionDetails(transactionId: string | bigint) {
  ...
  const targetTransaction = useMemo(() => {
    ...
    return baseTransaction.find((tx: SpherreTransaction) =>
  ...
  const {
    ...
    accountAddress!,
    transactionType === TransactionType.MEMBER_ADD ? transactionId : undefined!
  )

  const {
    data: memberRemovalData,
    isLoading: isLoadingMemberRemoval,
  } = useGetMemberRemovalTransaction(
    accountAddress!,
    transactionType === TransactionType.MEMBER_REMOVE ? transactionId : undefined!
  )
  ...
  const {
    ...
    accountAddress!,
    transactionType === TransactionType.MEMBER_PERMISSION_EDIT ? transactionId : undefined!
  )
  ...
  const {
    ...
    accountAddress!,
    transactionType === TransactionType.SMART_TOKEN_LOCK ? transactionId : undefined!
  )
